...
Whole third-party source trees are **deliberately not committed** to this repository:
they bloat git (~19k files) and duplicate code that already lives upstream. This folder
keeps only a **provenance manifest** of the external methods collected locally. Fetch the
actual source from the upstream URL / pip when a method is needed.
