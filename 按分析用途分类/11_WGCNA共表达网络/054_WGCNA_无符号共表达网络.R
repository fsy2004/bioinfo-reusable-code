...
# 脚本名     : WGCNA无符号共表达网络.R
# 分类       : 11_WGCNA共表达网络
# 项目来源   : 从压缩包 335.WGCNA分析更新，无符号网络方法.rar 整理
# 原始文件   : 335.WGCNA分析更新，无符号网络方法\GEOwgcna无符号网络.R
...
# ------------------------ 参数设置 ---------------------------
# 设置工作目录和输入文件
workDir <- "H:\\常用分析生信\\335.WGCNA分析更新，无符号网络方法"
...
par(cex = 0.8, mar = c(4, 6, 4, 2))  # 减少底部边距
plot(sampleDendro,
...
# 添加图例
legend("topright", legend = c("Control", "Disease"),
       col = c("#3498DB", "#E74C3C"), pch = 15, cex = 1.2)
dev.off()
...
for (module in unique_modules) {
  ...
  if (mm_column %in% colnames(geneModuleMembership)) {
    ...
    pdf(file.path("05_Gene_Analysis", paste0("MM_vs_GS_", module, "_module.pdf")),
        width = 6, height = 6)
    print(p_scatter)
    dev.off()
  }
  ...
  for (module in top_modules) {
    tryCatch({
      ...
      if (mm_col %in% colnames(geneModuleMembership)) {
        ...
        # 创建样本注释（将Treatment重新标记为Disease）
        display_groups <- ifelse(matched_groups == "Treatment", "Disease", matched_groups)
        sample_annotation <- data.frame(
          Group = factor(display_groups, levels = c("Control", "Disease"))
        )
        ...
        if (any(is.na(matched_groups))) {
          ...
          sample_annotation$Group <- factor(rep(c("Control", "Disease"),
                                               length.out = ncol(module_expr)))
        }

        # 注释颜色（使用新的标签）
        ann_colors <- list(
          Group = c(Control = "#3498DB", Disease = "#E74C3C")
        )
...
summary_stats <- list(
  ...
  control_samples = sum(clinicalData$Control),
  treatment_samples = sum(clinicalData$Disease),
