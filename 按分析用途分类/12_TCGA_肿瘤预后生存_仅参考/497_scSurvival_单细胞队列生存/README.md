# scSurvival: survival analysis from single-cell cohort data at cellular resolution

A scalable and interpretable tool for modeling survival outcomes from single-cell cohort data with cellular-resolution risk profiling.

## What is it

scSurvival models survival outcomes from single-cell cohort data and provides cellular-resolution risk profiling. It first employs a feature extraction module based on a variational autoencoder and generative modeling to learn batch-invariant single-cell representations, and then aggregates cell-level features to the patient level to perform multi-head attention-based multiple instance Cox regression. scSurvival integrates single-cell expression data with patient-level clinical variables to build survival risk prediction models, and it identifies key cell subpopulations most associated with survival risk and characterizes their risk tendencies for downstream analyses. The framework of scSurvival is depicted in the following figure:
