...
当前仓库包括 **12 份方法学参考、8 个 Python 工作流脚本、61 个 R 分析清单、15 个面向任务的 R 适配器
（另有 1 个公共适配器）和 26 个底层 R 工具模块**。61 个清单均已用声明的示例输入完成迁移验证；
...
Skill 会先识别综述类型和当前阶段，再只加载相关方法说明与工具；统计分析服从协议、估计目标、
研究设计、效应量、依赖结构和信息量，不以“工具齐全”为理由堆叠不必要分析。
