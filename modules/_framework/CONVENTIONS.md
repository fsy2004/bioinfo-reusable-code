...
## 0. 最高准则:复用而非从头写(防 AI 幻觉)

> **绝不从零手写分析脚本。** 任何分析都必须从既有代码起手:① 先查 `modules/CATALOG.md`
> 的「图类型→模块」反查表 + `TOOL_SELECTION_GUIDE.md` 选对模块/工具;② 库内无对应能力时,
> **找真实已发表工具的源码**(`gh search` / 记忆 `reference_github_search`,经 gh-proxy 克隆)再适配,
> **不臆造函数名/参数/方法步骤**。从头写 = 幻觉(假 API、错参数、统计/生物学不成立);
> 接地于真实工具代码 = 正确可复现。
>
> **推论:每个模块都必须有具体、可跑、有工具支撑的代码——占位/空壳目录是隐患**
> (会逼后续从头写 → 幻觉)。新增能力时,宁可建一个接地于真实工具的最小 turnkey 模块,
> 也不要留一个只有 README 的占位。对应记忆 `feedback_no_from_scratch`、`feedback_execution_workflow`
> (复用而非重写)、`feedback_no_unfounded_claims`(凡言有据)。
