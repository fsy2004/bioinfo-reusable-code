...
- **cCRE 词表 1,355,445**(`tokenization.tokenization` 的 `num_cCREs` 默认值,
  ...
  (`[CLS]=1` / `[SEP]=2` 见 `dataset.py:69` 的 `cell = [1] + cell.tolist() + [2]`;
  `[PAD]=0` 见 `dataset.py:80` 的 docstring 与 `dataset.py:93` 的 `value=0`);
...
由上游托管在 **Google Drive**(链接见上游 README 的更新日志与各任务小节,folder id `1WlNykSCNtZGsUp2oG0dw3cDdVKYDR-iX`),
**不是** GitHub release;仓库内 `model/EpiAgent-B/`、`model/EpiAgent-NT/` 目录只放了
`*_cell_types.json` 标签表,**不含权重**。仓库随包附带的是 `data/cCRE.bed` 与
`data/cCRE_document_frequency.npy`(上游 README 2026.05.28 更新日志里称作 `cCRE_frequency.npy`)。
端到端用法见上游 `demo/` 目录的 notebook —— 该目录**确实存在**且含 10 个 notebook
(`Data Preprocessing` / `Zero-shot Feature Extraction using EpiAgent` /
`Zero-shot annotation using EpiAgent-B` / `-NT` / `In-silico cCRE KO` /
`Fine-tuning EpiAgent for {unsupervised feature extraction, supervised cell type annotation,
data imputation, reference data integration and query data mapping, perturbation prediction}`),
本次已对本地克隆逐个 `ls` 确认文件名。
