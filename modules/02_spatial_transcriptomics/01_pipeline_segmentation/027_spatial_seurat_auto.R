...
library(viridis)
# 用户自定义参数
project_id <- "LUAD"    # 项目/样本名称
work_dir <- "C:/Users/fsy/Desktop/网络毒理学+单细胞+scTenifoldKnk模拟基因敲除+空间转录组+细胞轨迹+机器学习+分子对接+免疫分析+诊断模型/35.空间转录组数据分析"  # 工作目录
...
expr_matrix_dir <- "filtered_feature_bc_matrix"    # 存放表达矩阵和其他相关文件的文件夹路径
...
cat("Step 1/10: 检查空间数据文件...\n")  # 进度提示

# 检查必要文件是否存在
required_files <- c("spatial/tissue_hires_image.png", "spatial/tissue_lowres_image.png",
                    "spatial/scalefactors_json.json", "spatial/tissue_positions_list.csv")
missing_files <- required_files[!file.exists(required_files)]
if (length(missing_files) > 0) {
  cat("警告：缺少以下文件：", paste(missing_files, collapse = ", "), "\n")
  cat("请确保空间数据文件在工作目录中\n")
}


cat("Step 2/10: 读取表达矩阵...\n")  # 进度提示
# 读取.gz压缩格式的10X Genomics文件
barcodes <- read.delim(gzfile(file.path(expr_matrix_dir, "barcodes.tsv.gz")), header = FALSE, stringsAsFactors = FALSE)$V1
features <- read.delim(gzfile(file.path(expr_matrix_dir, "features.tsv.gz")), header = FALSE, stringsAsFactors = FALSE)
matrix_data <- Matrix::readMM(gzfile(file.path(expr_matrix_dir, "matrix.mtx.gz")))

# 处理重复的基因名
gene_names <- features[, 2]
# 如果有重复的基因名，添加基因ID作为后缀
if (any(duplicated(gene_names))) {
  gene_ids <- features[, 1]
  gene_names <- ifelse(duplicated(gene_names) | duplicated(gene_names, fromLast = TRUE),
                       paste0(gene_names, "_", gene_ids),
                       gene_names)
}
...
expr_matrix <- matrix_data

cat("Step 3/10: 读取空间图片和空间信息...\n")  # 进度提示
# 读取空间图片和空间信息
spatial_img <- Read10X_Image(
  image.dir = "spatial",      # spatial子文件夹
...
# 质量控制过滤：移除UMI计数为0或过低的细胞
cat("过滤低质量细胞...\n")
n_before <- ncol(sp_obj)
sp_obj <- subset(sp_obj, subset = nCount_Spatial > 0 & nFeature_Spatial > 0)
n_after <- ncol(sp_obj)
cat(sprintf("  过滤前: %d 个细胞, 过滤后: %d 个细胞, 移除: %d 个\n", n_before, n_after, n_before - n_after))

cat("Step 6/10: 数据标准化...\n")  # 进度提示
# 设置future全局变量大小限制为5GB，避免并行处理时内存溢出
...
expr_matrix <- as.data.frame(GetAssayData(sp_obj, layer = "counts"))  # 或 "data" 获取归一化数据
...
expr <- GetAssayData(sp_obj, layer = "data")
