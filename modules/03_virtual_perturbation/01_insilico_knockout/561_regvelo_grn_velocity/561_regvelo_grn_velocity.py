"""561 · RegVelo — gene-regulatory-informed RNA velocity & regulon perturbation.
...
def main():
    ...
    if a.h5ad:
        ...
        print("[561] no --h5ad given, using synthetic spliced/unspliced demo")
        adata = _synthetic()

    print("[561] baseline (no GRN): scVelo velocity + CellRank fates")
    b = baseline(adata, a.outdir)
    for k, v in b.items():
        print(f"       {k}: {v}")

    if a.run_regvelo:
        print("[561] RegVelo path")
        r = run_regvelo(adata)
        for k, v in r.items():
            print(f"       {k}: {v}")
    else:
        print("[561] RegVelo path not requested (--run-regvelo); baseline only")

    import json
    with open(os.path.join(a.outdir, "561_summary.json"), "w") as fh:
        json.dump({"baseline": b}, fh, indent=1, default=str)
    print(f"[561] wrote {os.path.join(a.outdir, '561_summary.json')}")
