...
> ⚠️ **诚实标注**:上游的距离项由 `pertpy.tools.Distance` 计算(mse / wasserstein /
...
- 给深度扰动预测的结果做**必备对照**——这正是 scArchon 论文的核心论点
  (方法常常打不过朴素基线,论文原题即"生物学幻觉");
...
合成数据上的读数(示例):`linear` delta-R²(top100)≈ 0.52,`control` ≈ −0.02,
`global_mean` ≈ 0.39 —— 即加性基线确实学到了可迁移的扰动响应,而只背均值的退化解在
距离类指标上反而比不做预测还差。
...
仓库:https://github.com/hdsu-bioquant/scArchon(MIT)。
实际读取接口的 URL(2026-07-20):`README.md`、`config/datasets.tsv`、
`scripts/metrics/metrics.py`、`scripts/linear/snkmk_linear.py`(均为 `main` 分支 raw 文件)。
