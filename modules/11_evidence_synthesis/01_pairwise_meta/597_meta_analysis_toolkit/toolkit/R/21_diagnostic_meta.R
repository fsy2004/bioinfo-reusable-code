...
dta_run <- function(data, out_prefix = "figures/21", add_correction = 0.5) {
  ...
  ## ---- figures (all non-bar) ----
  ff <- function(suffix, expr) { grDevices::pdf(paste0(out_prefix, suffix), width = 7, height = 6.2)
  ...
  grDevices::pdf(paste0(out_prefix, "_sens.pdf"), width = 7, height = 5.5)
  mada::forest(descr, type = "sens"); grDevices::dev.off()
  grDevices::pdf(paste0(out_prefix, "_spec.pdf"), width = 7, height = 5.5)
